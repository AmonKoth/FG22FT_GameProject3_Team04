...
void APlayerCharacter::Dash()
{
	...
	{
		const FVector InputDirection = MoveAxisVector.Y * UKismetMathLibrary::GetForwardVector(GetActorRotation()) +
								   MoveAxisVector.X * GetActorRightVector();
	...
	else if (MoveAxisVector.Length() > 0 && bIsInCombat)
	{
		...
		auto TransformedDirection = UKismetMathLibrary::TransformDirection(GetActorTransform(), DashDirection);

		//xSign > 0 ? DashDirection = GetActorRightVector() : DashDirection = -GetActorRightVector();
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Magenta, DashDirection.ToString());
	...
	if (bCanDash)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Magenta, DashVelocity.ToString());
	}
	else
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Magenta, "Can't dash yet!");
	}

	//bIsDashing = true; // Set the dashing flag to true
}
